  ...
  return (
    <Stack direction="row" justifyContent="start" alignItems="start">
      <Stack direction="row" spacing={ 1 } justifyContent="start" alignItems="center">
        <Image src={ `${POKEMON_SPRITE_API_URL}${index}.png` } alt={ name } width={ 50 } height={ 50 } />
        <Stack direction="column">
          <AppLink href={ urlLink } title={ <Typography>{ startCase(name) }</Typography> } />
          <Stack direction="row" justifyContent="start" alignItems="center" spacing={ 1 }>
            <FavoritePokemonButton pokemonName={ name } isFavorite={ !!favorite } />
          </Stack>
        </Stack>
      </Stack>
    </Stack>
