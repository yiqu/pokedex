...
export async function getCommentListByGameVersion(version: string): Promise<Comment[]> {
  ...
  //   await new Promise((resolve) => setTimeout(resolve, 1000));
  // }
  const commentsResponse = await fetch(`${FIREBASE_API_URL}next/comments/${version}/.json`, { next: { tags: [`game-comments-${version}`] } });
  ...
  const commentListWithFireKey = Object.keys(comments ?? {}).map((key) => {
    ...
    const commentDate = cDate ? formatInTimeZone(new Date(cDate), EST_TIME_ZONE, 'MM/dd/yy hh:mm a OOOO') : 'no date found.';
    return {
      ...comments[key],
      fireKey: key,
